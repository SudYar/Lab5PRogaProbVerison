...
public class ReplaceIfLowe extends AbstractCommand {
    ...
    public boolean execute(String argument) {
        ...
        try {
            ...
            if (!studyGroupCollection.getCollection().containsKey(id)) throw new IllegalArgumentException("Такого Id нет в коллекции");
            System.out.println("Меняем элемент коллекции:");
            ...
            StudyGroup studyGroup = new StudyGroup(id, name, new Coordinates(x, y), studentCount, formOfEducation, semester, groupAdmin);
            if (studyGroup.compareTo(studyGroupCollection.getById(id)) < 0){
                studyGroupCollection.update(id,
                        name, x, y, studentCount, formOfEducation, semester,
                        groupAdmin);
                System.out.println("Группа изменена");
            } else System.out.println("Заданная группа превышает старую, коллекция не изменилась" );
